...
First, we need to derive a spatial weight matrix before we can compute local Gi\* statistics. Since local Gi\* need to use distance matrix, hence distance matrix is used.

Since the geometry field in drug_use_cases is multipolygon, instead of polygon. I will use st_centroid to get the central point.
